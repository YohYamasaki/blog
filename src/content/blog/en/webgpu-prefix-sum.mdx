---
title: "Prefix Sum on WebGPU: from Hillis–Steele, Blelloch, to Subgroups"
...
Given an array $A$, a prefix sum array $S$ is defined as $S[i] = S[i-1] + A[i]$. In other words, $S[i]$ contains the sum of $A[0..i]$ for inclusive scan or $A[0..i-1]$ for exclusive scan. Prefix sums show up everywhere, i.e. radix sort, quadtrees, and a bunch of other “build a structure from counts/offsets” workloads.
...
Hillis–Steele is an iterative algorithm where the dependency distance doubles each step.
If you draw a binary-tree-like reduction that computes $S[i]$ from leaves $A[0..i]$, you’ll notice that at each height you only depend on two values, which is a good start for parallel execution.
...
WebGPU provides uniform buffers for this kind of small “constant-ish” data.
...
Ideally `timestamp-query` should be the best way for this benchmark, but on Metal I couldn’t get it to work for some reasons, so I measure from `submit` until the device becomes usable again. This includes a polling overhead (on my machine 500µs to 1ms), but that’s good enough if we compare at large $N$.
...
Hillis–Steele has an ideal parallel time of $O(\log N)$, so it sounds like it could be faster than $O(N)$ sequential scan. But in practice it’s bottlenecked by memory bandwidth and work inefficiency.

Let’s do a rough bandwidth-based estimate. On M4 Pro, the unified memory bandwidth is advertised as 273 GB/s.
If $N = 10^8$ and we store `u32`, the array size is $10^8 \times 4$ bytes = $400,000,000$ bytes $\approx$ 400 MB. Hillis–Steele needs $\lceil \log_2 N \rceil = 27$ steps. With double buffering, each step touches the whole array with one read and one write → about 2× array traffic per step.

- Total transfers ≈ $400\text{ MB} \times 54 = 21.6\text{ GB}$
- Ideal bandwidth time ≈ $21.6 / 273 \approx 0.0791$ s ≈ 79.1 ms

In my measurement at $N = 10^8$, it took 104.36 ms, which is consistent with “bandwidth dominates, plus overheads”.

Also, if you execute Hillis–Steele on a single thread, the total work is $O(N \log N)$, which is not optimal compared to $O(N)$ algorithms.
...
Much better than Hillis–Steele, but still not ideal. Even though we reduced work, this implementation still hits global memory a lot (roughly $7N$ reads/writes on my implementation I guess), so there’s room to improve.
